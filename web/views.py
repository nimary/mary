...
postmark = PostmarkClient(server_token='d1864b33-1681-4512-848d-7808137fa3cc')

# create random string for Toekn
random_str = lambda N: ''.join(
    random.SystemRandom().choice(str.ascii_uppercase + str.ascii_lowercase + str.digits) for _ in range(N))
...
def login(request):
    ...
    if request.POST.__contains__('username') and request.__contains__('password'):
...
def register(request):
    if request.POST.__contains__(
        ...
        if not User.objects.filter(username=request.POST['username']).exists():
            ...
            temporarycode.save()
            message = postmark.emails.send(
                From ="n.rezaei@nioec.ir",
                To=email,
                Subject="فعالسازی اکانت بستون",
                TextBody=' برای فعال کردن اکانت بستون خود روی لینک روبرو کلیک کنید: {}?code={}'.format(
